...
async function acquireWakeLock(){
  if(!('wakeLock' in navigator)||!playing||document.visibilityState!=='visible'||wakeLock)return;
  try{const lock=await navigator.wakeLock.request('screen');if(!playing){await lock.release();return;}wakeLock=lock;lock.addEventListener('release',()=>{if(wakeLock===lock)wakeLock=null;});}catch{/* Screen wake lock is optional. */}
}
function stop(){
...
async function start(){
  ...
  try{
    ...
    if(!AudioContext)throw new Error('Audio is not supported in this browser. Try Safari, Chrome, Firefox, or Edge.');
    if(!context||context.state==='closed'){
      context=new AudioContext();
      context.onstatechange=()=>{if(playing&&context.state!=='running'){stop();$('status').textContent='Paused';$('play-hint').textContent='Audio was interrupted. Tap Play to resume.';}};
    }
